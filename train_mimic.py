...
def setup_mimic_training():
    ...
    return get_args(defaults={
        ...
        'hyper_cls_inter': 3.0,
        'hyper_quan': 0.1,
        'data_path': 'dataset/mimic-cxr',
        'result_name': 'result/Result_TriPAH_MIMIC',
        'device': 'cuda',
        'is_train': True,
    })

def check_mimic_dataset(data_path):
    """检查MIMIC-CXR数据集是否存在"""
    ...
    if missing_files:
        print("❌ MIMIC-CXR数据集文件缺失:")
        for file_name in missing_files:
            print(f"   - {file_name}")
        print("\n💡 请先运行转换脚本:")
        print("   python -m dataset.builders.convert_mimic_dataset")
        return False

    print("✅ MIMIC-CXR数据集文件检查通过")
    return True

def load_mimic_dataset(args):
    """加载MIMIC-CXR数据集"""
    print("📊 加载MIMIC-CXR数据集...")

    try:
        # 解析数据文件路径：若为绝对路径或已存在则直接使用；否则拼接到 dataset/mimic-cxr 下
        def _resolve(p: str) -> str:
            try:
                ...
                pass
            return os.path.join("dataset", "mimic-cxr", p)
...
def train_mimic_model(args, train_data, query_data, retrieval_data):
    """训练MIMIC-CXR模型"""
    print("🚀 开始训练MIMIC-CXR模型...")
    ...
    try:
        ...
        print(f"   医学图像特化参数:")
        print(f"     - 类间损失权重: {args.hyper_cls_inter}")
        print(f"     - 量化损失权重: {args.hyper_quan}")
...
def evaluate_mimic_model(args, query_data, retrieval_data):
    """评估MIMIC-CXR模型"""
    print("📈 评估模型性能...")
...
def main():
    """主函数"""
    print("=" * 60)
    print("🏥 MIMIC-CXR数据集训练 - TriPAH")
    print("=" * 60)

    # 解析参数
    args = setup_mimic_training()
    ...
    # 检查数据集
    if not check_mimic_dataset(args.data_path):
        return

    # 加载数据集
    train_data, query_data, retrieval_data = load_mimic_dataset(args)
    if train_data is None:
        return

    # 训练模型
    training_success = train_mimic_model(args, train_data, query_data, retrieval_data)
    ...
    if training_success:
        ...
        print("🔍 开始模型评估")
        print("=" * 60)
        evaluate_mimic_model(args, query_data, retrieval_data)
    else:
        print("\n⚠️  由于训练失败，跳过模型评估")
